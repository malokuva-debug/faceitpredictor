...
app.get('/api/auth/faceit', (req, res) => {
  ...
  const code_challenge = req.query.code_challenge || ''; // generated by extension
  ...
  // Redirect the user to FACEIT login
...
app.get('/popup-success', async (req, res) => {
  ...
  if (!code || !code_verifier) return res.send('❌ Missing code or verifier');

  try {
    // Exchange code + code_verifier for FACEIT access token
    ...
    // Sign your own JWT for your extension
    const jwtToken = jwt.sign({ faceitToken: data.access_token }, JWT_SECRET, { expiresIn: '24h' });

    // Return HTML to popup that stores token & closes
    res.send(`
      <script>
